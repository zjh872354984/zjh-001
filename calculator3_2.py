...
class UserData(object):
    ...
    def calculator(self):
        ...
        income_dict = {}
        baobiao_list = []
        for ud_k in self.userdata.keys():
            self.userdata[ud_k] = float(self.userdata[ud_k])
            if self.userdata[ud_k] < jishul:
                jishu = jishul
            elif self.userdata[ud_k] > jishuh:
                jishu = jishuh
            else:
                jishu = self.userdata[ud_k] 
            
            shebao = jishu * tax_total
            tax0 = self.userdata[ud_k] - shebao -3500
            if tax0<=0:
                tax0 = 0
                tax = 0
            elif tax0<=1500:
            ...
            income = self.userdata[ud_k] - tax - shebao
            income_dict[ud_k] = format(income, '0.2f')
            self.userdata[ud_k] = format(self.userdata[ud_k], '0.2f')
           
            baobiao_list.append(ud_k,userdata[ud_k],shebao_dict[ud_k],
